...
public:
	...
	Airplane()
	{
		NumberFlights = "";
		NumberPassengers = 0;
		cout << "Применён конструктор по умолчанию" << endl;
	...
	{
		NumberFlights = move(moved.NumberFlights);      // перемещаем кол-во пассажиров
		NumberPassengers = move(moved.NumberPassengers);// перемещаем номер рейса
	...
	Airplane& operator=(Airplane&& moved) noexcept 
	{
		if (this != &moved) 
		{
			NumberFlights = move(moved.NumberFlights);
			NumberPassengers = move(moved.NumberPassengers);
			moved.NumberPassengers = 0;
		}
		return *this;
	}
